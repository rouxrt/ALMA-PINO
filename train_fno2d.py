...
def evaluate_model(model, dataloader, criterion, device, show_datacube=False):
    ...
    with torch.no_grad(): 
        for dirty, clean, psf in dataloader:
            ...
            if show_datacube:
                visualize_datacube(dirty, clean, pred_clean, output_dir="training_results/results_FNO2D/visualizations", model_name="FNO2D")
                plot_spectral_profile(clean, pred_clean, sample_idx=0, output_dir="training_results/results_FNO2D/", model_name="FNO2D")
                show_datacube = False
            loss_total, l1, msssim = criterion(pred_clean, dirty, clean, psf)
...
def main(args):
    ...
    if not tuning_mode:
        print(f"Starting FNO2D training on device: {device}")
    ...
    for epoch in range(1, args.epochs + 1):
        ...
        if (epoch % 5 == 0 or epoch == args.epochs) and not tuning_mode:
            ...
            save_predictions_FNO(sample_dirty, sample_clean, sample_pred, 
                                 epoch, tot_loss, l1, msssim, output_dir="training_results/results_FNO2D/predictions", dim="2D")
    
    if not tuning_mode:
        plot_loss_history_FNO(history_loss, title="FNO2D Training Loss", save_path="training_results/results_FNO2D/loss_history.png")
