...
@torch.no_grad()
def infer_lno2d(model, dirty, device):
    return torch.clamp(model(dirty.to(device)), min=0.0)


@torch.no_grad()
def infer_lno3d(model, dirty, device):
...
def run_benchmark(args):
    ...
    for sample_idx, (dirty, clean, psf) in enumerate(test_loader):
        ...
        for name, (model, is_3d, do_tto, tto_cfg) in models_to_run.items():
            infer_fn = infer_lno2d if not is_3d else infer_lno3d
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(description="Benchmark LNO2d/3d/PI (pre+post TTO) vs CLEAN")

    parser.add_argument("--lno2d",   type=str, default="C:/Users/rober/OneDrive/Desktop/Tesi/best_lno_models/lno2d.pth", help="Path checkpoint LNO2d")
    parser.add_argument("--lno3d",   type=str, default="C:/Users/rober/OneDrive/Desktop/Tesi/best_lno_models/lno3d.pth", help="Path checkpoint LNO3d")
    parser.add_argument("--pilno2d", type=str, default="C:/Users/rober/OneDrive/Desktop/Tesi/best_lno_models/pilno2d.pth", help="Path checkpoint PI-LNO2d")
    parser.add_argument("--pilno3d", type=str, default="C:/Users/rober/OneDrive/Desktop/Tesi/best_lno_models/pilno3d.pth", help="Path checkpoint PI-LNO3d")

    # LNO2D
    parser.add_argument("--modes_lno2d", type=int,   default=12)
    parser.add_argument("--width_lno2d", type=int,   default=64)

    # LNO3D
    parser.add_argument("--modes_lno3d", type=int,   default=8)
    parser.add_argument("--modes_z_lno3d", type=int, default=6)
    parser.add_argument("--width_lno3d", type=int,   default=32)
  

    # PI-LNO2D
    parser.add_argument("--modes_pilno2d", type=int,   default=12)
    parser.add_argument("--width_pilno2d", type=int,   default=64)

    # PI-LNO3D
    parser.add_argument("--modes_pilno3d", type=int,   default=8)
    parser.add_argument("--modes_z_pilno3d", type=int, default=6)
    ...
    parser.add_argument("--tto_lr_pilno2d",     type=float, default=1e-7,
    ...
    # Output
    parser.add_argument("--output_dir", type=str, default="results_benchmark_lno")
