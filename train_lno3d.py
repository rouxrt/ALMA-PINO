...
def evaluate_model(model, dataloader, criterion, device, show_datacube=False):
    ...
    with torch.no_grad(): 
        for dirty, clean, psf in dataloader:
            ...
            if show_datacube:
                visualize_datacube(dirty, clean, pred_clean, output_dir="results_LNO3D/datacube_visualization", model_name="LNO3D")
                plot_spectral_profile(clean, pred_clean, sample_idx=0, output_dir="results_LNO3D/", model_name="LNO3D")
                show_datacube = False
...
def main(args):
    set_seed(42)
    os.makedirs('results_LNO3D', exist_ok=True)
    tuning_mode = hasattr(args, 'trial') and args.trial is not None
    
    if not tuning_mode:
        sys.stdout = Logger(f"results_LNO3D/training_log.txt")
    ...
    for epoch in range(1, args.epochs + 1):
        ...
        if (epoch % 5 == 0 or epoch == args.epochs) and not tuning_mode:
            ...
            save_predictions_FNO(sample_dirty, sample_clean, sample_pred.cpu(), 
                             epoch, tot_loss, l1, msssim, output_dir="results_LNO3D/predictions", dim="3D")

    if not tuning_mode:        
        plot_loss_history_FNO(history_loss, title="LNO3d Training Loss", save_path="results_LNO3D/loss_history.png")
